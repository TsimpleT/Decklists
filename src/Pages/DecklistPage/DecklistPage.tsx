...
export class DecklistPage extends React.Component<IProps> {
    ...
    public constructor(props: IProps) {
        ...
        this.username = url.slice(url.indexOf("decklist/")+9, url.length-1);
    ...
    public render(): React.ReactNode {
        return ((!this.decklist || !this.placing) ? <>decklist not found</> : <>
            <VDecklist decklist={this.decklist} title={`${this.username}: ${GET_TOURNAMENT_NAME(this.tournId)} ${this.decklist.date} ${this.placing}`} />
        </>);
    }
}
